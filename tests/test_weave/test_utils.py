...
def test_format_model_name_correctly_formats_inspect_valid_model_names(
    model_name: str,
) -> None:
...
class TestFormatScoreTypes:
    ...
    def test_multi_element_sequence_raises_error(self):
        ...
        with pytest.raises(
            ValueError, match="Sequence score cannot be passed to Weave"
        ):
    ...
    def test_complex_nested_mapping(self):
        ...
        input_dict = {"metrics": {"accuracy": 0.85, "precision": 0.90}, "score": 0.87}
        result = format_score_types(input_dict)
        assert result == input_dict
...
class TestFormatSampleDisplayName:
    ...
    @pytest.mark.parametrize(
        "template,task_name,sample_id,epoch,expected",
        [
