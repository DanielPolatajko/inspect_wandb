...
class TestWandBModelHooksE2ERetryScenarios:
    ...
    def test_wandb_uses_same_run_id_across_retries_with_eval_set(
        ...
        with (
            ...
            patch(
                "inspect_wandb.weave.hooks.WeaveSettings.model_validate"
            ) as mock_weave_settings,
        ):
            ...
            mock_weave_settings.return_value = WeaveSettings(enabled=False)
    ...
    def test_wandb_uses_same_run_id_when_rerunning_same_log_dir(
        ...
        with (
            ...
            assert first_run_id == second_run_id, (
                f"Expected same run_id for same log_dir, got {first_run_id} != {second_run_id}"
            )
    ...
    def test_wandb_uses_different_run_id_for_different_log_dirs(
        ...
        with (
            ...
            second_run_id = mock_wandb_init.call_args_list[0][1]["id"]
            assert first_run_id != second_run_id, (
                f"Expected different run_ids for different log_dirs, got {first_run_id} == {second_run_id}"
            )
    ...
    def test_hooks_disabled_no_wandb_init_during_retries(
        ...
        with (
            ...
            disabled_settings = ModelsSettings(
                enabled=False, entity="test-entity", project="test-project"
            )
