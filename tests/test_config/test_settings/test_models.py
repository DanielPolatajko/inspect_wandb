...
class TestModelsSettings:
    ...
    def test_errors_for_invalid_environment_variables(
        ...
        monkeypatch.setenv(
            "INSPECT_WANDB_MODELS_ENVIRONMENT_VALIDATIONS",
            '{"wandb_base_url": "https://api.wandb.ai", "wandb_api_key": "1234567890"}',
        )
    ...
    def test_passes_validations_when_environment_variables_are_valid(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # Given
        ...
        assert (
            models_settings.environment_validations.wandb_base_url
            == "https://api.wandb.ai"
        )
        assert models_settings.environment_validations.wandb_api_key == "1234567890"

    def test_complete_priority_order(
    ...
    def test_config_field_serialization(
        self, monkeypatch: pytest.MonkeyPatch, initialise_wandb: None
    ) -> None:
        # Given

        config_json = (
            '{"learning_rate": 0.001, "batch_size": 32, "nested": {"value": true}}'
        )
