...
```{note}
Weave calls these **conversations** (the SDK was renamed from *session* to *conversation* in weave 0.53), and each one appears under the **Conversations** tab as well as in the Agents view. Inspect WandB keeps the term *agent session*, since an Inspect sample is a scored agent rollout rather than a chat, and so that the `agent_sessions` setting does not have to change every time Weave revises its vocabulary. The two names refer to the same object: one sample is one session is one conversation.
```
