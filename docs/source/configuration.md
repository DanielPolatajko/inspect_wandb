...
| **AGENT_SESSIONS** | When `true`, streams each sample's agent trajectory to Weave's agent Conversation SDK (the [Agents view](https://docs.wandb.ai/weave/guides/tracking/view-agent-activity)) as turns complete, in addition to eval logging. Each sample appears as one *conversation* in Weave (see {doc}`concepts`). Enables live, turn-level observability and server-side Monitors/Signals on long-horizon agentic evals. See {doc}`demo-agent-monitoring`. | `False` |
