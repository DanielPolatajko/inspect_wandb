...
async def patched_task_run_sample(
    *args, **kwargs
    ...
    patched_plan = PatchedPlan(
        plan.steps, plan.finish, plan.cleanup, plan.name, internal=True
    )
    patched_scorers: list[Scorer] | None = (
        [PatchedScorer(scorer) for scorer in scorers] if scorers else None
    )

    return await task_run_sample(
        *args, plan=patched_plan, scorers=patched_scorers, **kwargs
    )
